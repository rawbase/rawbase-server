...
public class RawbaseSPARQLServer extends SPARQLServer {
    ...
    public RawbaseSPARQLServer(ServerConfig config) {
        super(config);
        ...
        ServletContextHandler context = buildServer(getServerConfig().jettyConfigFile, config.enableCompression) ;
        configureDatasets(context) ;
    }
    
    private void configureDatasets(ServletContextHandler context) {
        // Build them all.
        for (DatasetRef dsDesc : getServerConfig().datasets)
            configureOneDataset(context, dsDesc, getServerConfig().enableCompression) ;
    ...
    private ServletContextHandler buildServer(String jettyConfig, boolean enableCompression) {
        if ( jettyConfig != null ) {
            ...
            server = defaultServerConfig(getServerConfig().port, getServerConfig().loopback) ;
        ...
        if ( jettyConfig == null && getServerConfig().authConfigFile != null ) {
            ...
            HashLoginService loginService = new HashLoginService("Fuseki Authentication", getServerConfig().authConfigFile) ;
            ...
            serverLog.debug("Basic Auth Configuration = " + getServerConfig().authConfigFile) ;
        }
        ...
        serverLog.debug("Pages = " + getServerConfig().pages) ;
        ...
        if ( installManager || installServices ) {
            // TODO Respect port.
            if ( getServerConfig().pagesPort != getServerConfig().port )
                serverLog.warn("Not supported yet - pages on a different port to services") ;
             if ( getServerConfig().pages != null ) {
                 if ( ! FileOps.exists(getServerConfig().pages) )
                     serverLog.warn("No pages directory - "+getServerConfig().pages) ;
                String base = getServerConfig().pages ;
        ...
        if ( installManager || installServices ) {
            ...
            addContent(context, "/", getServerConfig().pages) ;
        }
    ...
    private void configureOneDataset(ServletContextHandler context, DatasetRef sDesc, boolean enableCompression) {
        ...
        if (sDesc.dataset instanceof RawbaseDataSet.RawbaseDataSetGraph) {
            sparqlQuery = new SPARQL_QueryRawbase();
            sparqlUpdate = new SPARQL_UpdateRawbase();
        } else {
            sparqlQuery = new SPARQL_QueryDataset();
            sparqlUpdate = new SPARQL_Update();
        }
        ...
        if (!überServlet) {
            ...
            addServlet(context, datasetPath, sparqlQuery, sDesc.query, enableCompression);
            addServlet(context, datasetPath, sparqlUpdate, sDesc.update, false);
            addServlet(context, datasetPath, sparqlUpload, sDesc.upload, false);    // No point - no results of any size.
            addServlet(context, datasetPath, sparqlHttpR, sDesc.readGraphStore, enableCompression);
            addServlet(context, datasetPath, sparqlHttpRW, sDesc.readWriteGraphStore, enableCompression);
